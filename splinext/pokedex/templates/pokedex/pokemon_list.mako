...
<ul class="classic-list">
    ...
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='1', sort='evolution-chain')}">${dexlib.generation_icon(1)} ${_(u"Red/Blue/Yellow")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='2', sort='evolution-chain')}">${dexlib.generation_icon(2)} ${_(u"Gold/Silver/Crystal")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='3', sort='evolution-chain')}">${dexlib.generation_icon(3)} ${_(u"Ruby/Sapphire/Emerald")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='4', sort='evolution-chain')}">${dexlib.generation_icon(4)} ${_(u"Diamond/Pearl/Platinum")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='5', sort='evolution-chain')}">${dexlib.generation_icon(5)} ${_(u"Black/White")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='6', sort='evolution-chain')}">${dexlib.generation_icon(6)} ${_(u"X/Y")}</a></li>
    <li><a href="${url(controller='dex_search', action='pokemon_search', introduced_in='7', sort='evolution-chain')}">${dexlib.generation_icon(7)} ${_(u"Sun/Moon")}</a></li>
</ul>

<h2>${_(u"By regional Pokédex")}</h2>
<ul class="classic-list">
    <li><a href="${url(controller='dex_search', action='pokemon_search', in_pokedex='7', sort='evolution-chain')}">${dexlib.generation_icon(2)} ${_(u"Johto")}</a></li>
